...
import contextvit.data as data
import contextvit.utils as utils
from contextvit.meta_arch import DINO
...
class LinearProb(pl.LightningModule):
    def __init__(self, main_linear_prob_cfg: DictConfig) -> None:
        ...
        if self.cfg.checkpoint != None:
            ...
            self.model = DINO.load_from_checkpoint(self.cfg.checkpoint)
        ...
        if self.cfg.use_mlp:
            self.classifier = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Linear(self.model.embed_dim, self.cfg.num_classes),
            )
        else:
            self.classifier = nn.Linear(self.model.embed_dim, self.cfg.num_classes)
